...
const BoardAssets = (() => {
  ...
  register("stick_figure",
    ["man", "person", "human", "boy", "girl", "woman", "people",
     "student", "teacher", "figure", "stickman", "stick_man", "walker"],
    ({ color = "#E6EDF3", height = 2.0 } = {}) => {
      const s = height / 2.7, lw = 4;            // native span ~2.7 units tall
      return [
        { k: "circle", x: 0, y: 1.3 * s, r: 0.3 * s, stroke: color, lw },
        { k: "line", x1: 0, y1: 1.0 * s, x2: 0, y2: -0.2 * s, stroke: color, lw },
        { k: "line", x1: -0.6 * s, y1: 0.6 * s, x2: 0.6 * s, y2: 0.6 * s, stroke: color, lw },
        { k: "line", x1: 0, y1: -0.2 * s, x2: -0.45 * s, y2: -1.1 * s, stroke: color, lw },
        { k: "line", x1: 0, y1: -0.2 * s, x2: 0.45 * s, y2: -1.1 * s, stroke: color, lw },
      ];
    });

  register("road", ["street", "path", "highway", "ground", "floor"],
    ({ color = "#30363D", width = 14.0, height = 1.6 } = {}) => {
      ...
      for (let x = -width / 2 + 0.7; x < width / 2 - 0.5; x += 1.4) {
        prims.push({ k: "line", x1: x - 0.35, y1: 0, x2: x + 0.35, y2: 0,
                     stroke: "#E3B341", lw: 6 });
      }
      return prims;
    });
  ...
  register("house", ["home"], ({ color = "#D29922" } = {}) => [
    ...
    { k: "poly", pts: [[-1.0, 0.6], [1.0, 0.6], [0, 1.5]], fill: "#B62324", closed: true },
  ...
  register("sun", [], ({ color = "#F2CC60" } = {}) => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      prims.push({ k: "line", x1: c * 0.8, y1: s * 0.8, x2: c * 1.2, y2: s * 1.2,
                   stroke: color, lw: 5 });
    }
    return prims;
  });
  ...
  register("mountain", ["hill"], ({ color = "#6E7681" } = {}) => [
    { k: "poly", pts: [[-1.5, -0.8], [1.5, -0.8], [0, 1.2]], fill: color, closed: true },
  ]);
  ...
  register("building", ["tower", "skyscraper"], ({ color = "#484F58" } = {}) => {
    ...
    for (let row = 0; row < 5; row++)
      for (let col = 0; col < 2; col++)
        prims.push({ k: "rect", x: -0.35 + col * 0.7, y: 1.0 - row * 0.5,
                     w: 0.3, h: 0.3, fill: "#F2CC60" });
    return prims;
  });
  ...
  return { build, names: () => Object.keys(FACTORIES) };
})();
